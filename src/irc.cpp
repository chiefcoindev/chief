...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("TheChiefCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #TheChiefCoinTEST2\r");
            Send(hSocket, "WHO #TheChiefCoinTEST2\r");
        } else {
            // randomly join #TheChiefCoin00-#TheChiefCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #TheChiefCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #TheChiefCoin%02d\r", channel_number).c_str());
        }
